m.infos.nest.aovlist <- function(x,
                                 my,
                                 forminter,
                                 which,
                                 fl1,
  ...
  if (is.null(fl2)) {
    f2 <- levels(model.frame(x)[, nf1])[fl1] # corresponde ao fator onde se esta fazendo o desdobramento!

    aux_m.inf21 <- .tukeyc_filter_nest(aux_m.inf2, which, fl1, fl2, model.frame(x))
