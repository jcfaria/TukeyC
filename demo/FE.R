##
## Example: Factorial Experiment (FE)
##

## The parameters can be: design matrix and the response variable,
## data.frame or aov

## Note: Upper case for factors and lowercase for levels

library(TukeyC)
data(FE)

## From: design matrix (dm) and response variable (y)
## Main factor: N
tk1 <- with(FE, TukeyC(x=dm, y=y, model='y ~ blk + N*P*K', 
                       which='N'))
summary(tk1)
plot(tk1)

## Main factor: P
tk2 <- with(FE, TukeyC(x=dm, y=y, model='y ~ blk + N*P*K',
                       which='P'))
summary(tk2)

## Main factor: K
tk3 <- with(FE, TukeyC(x=dm, y=y, model='y ~ blk + N*P*K',
                       which='K'))
summary(tk3)

## Nested: p1/N
ntk1 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='N:P', fl2=1))
summary(ntk1)
plot(ntk1)

## Nested: p2/N
ntk2 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='N:P', fl2=2))
summary(ntk2)
plot(ntk2)

## Nested: k1/N
ntk3 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='N:K', fl2=1))
summary(ntk3)

## Nested: k2/N
ntk4 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='N:K', fl2=2))
summary(ntk4)

## Nested: k1/P
ntk5 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='P:K', fl2=1))
summary(ntk5)

## Nested: k2/P
ntk6 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='P:K', fl2=2))
summary(ntk6)

## Nested: k1/p1/N
ntk7 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='N:P:K', fl2=1, fl3=1))
summary(ntk7)

## Nested: k2/p2/N
ntk8 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + N*P*K',
                             which='N:P:K', fl2=2, fl3=2))
summary(ntk8)
plot(ntk8)

## Nested: k1/n1/P
ntk9 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + P*N*K',
                             which='P:N:K', fl2=1, fl3=1))
summary(ntk9)

## Nested: k2/n2/P
ntk8 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + P*N*K',
                             which='P:N:K', fl2=2, fl3=2))
summary(ntk8)

## Nested: p1/n1/K
ntk10 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + K*N*P',
                              which='K:N:P', fl2=1, fl3=1))
summary(ntk10)

## Nested: p2/n2/K
ntk11 <- with(FE, TukeyC.nest(x=dm, y=y, model='y ~ blk + K*N*P',
                              which='K:N:P', fl2=2, fl3=2))
summary(ntk11)


## From: data.frame
## Nested: k2/p1/N
ntk12 <- with(FE, TukeyC.nest(x=dfm, model='y ~ blk + N*P*K',
                              which='N:P:K', fl2=1, fl3=2))
summary(ntk12)

## Nested: k1/p2/N
ntk13 <- with(FE, TukeyC.nest(x=dfm, model='y ~ blk + N*P*K',
                              which='N:P:K', fl2=2, fl3=1))
summary(ntk13)


## From aov
nav1 <- with(FE, aov(y ~ blk + N*P*K , data=dfm))
summary(nav1)

## Main factor: N
ntk14 <- TukeyC(nav1,
                which='N')
summary(ntk14)

## Nested: k1/P
ntk15 <- TukeyC.nest(nav1,
                     which='P:K', fl2=1)
summary(ntk15)

## Nested: k2/p1/N
ntk16 <- TukeyC.nest(nav1,
                     which='N:P:K', fl2=1, fl3=2)
summary(ntk16)

# Changing the order of factors (test)
nav2 <- with(FE, aov(y ~ blk + K*N* P, data=dfm))
summary(nav2)

## Nested: p1/n1/K
ntk17 <- TukeyC.nest(nav2,
                     which='K:N:P', fl2=1, fl3=1)
summary(ntk17)
